...
function AddNewItemContent() {
  ...
  const requestedMrnId = useMemo(() => {
    return searchParams.get("mrnId");
  }, [searchParams]);
  ...
  useEffect(() => {
    ...
    async function loadMrn() {
      try {
        ...
        const firstMrn = Array.isArray(firstPage?.rows)
          ? firstPage.rows[0]
          : null;

        if (isMounted) {
          setMrnMeta(firstMrn || null);
        }
      } catch (error) {
        console.error("Failed to load MRN:", error);
  ...
  return (
    <div className="bg-gray-100 pt-6 pr-6 pl-6">
      ...
      <div className="bg-white rounded-sm shadow p-4 space-y-4">
        ...
        {mrnMeta?.mrnId ? (
          <ItemsTable mrnId={mrnMeta.mrnId} />
        ) : (
          <div>Loading...</div>
        )}
